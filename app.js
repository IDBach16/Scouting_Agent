...
function appendMessage(role, text, charts) {
  ...
  if (role==='assistant') {
    ...
    // Big "Export PDF" button at the bottom — only for full-analysis output
    if (appMode === 'full') {
      const pdfBar = document.createElement('div');
      pdfBar.className = 'pdf-export-bar';
      const pdfBtn = document.createElement('button');
      pdfBtn.className = 'pdf-export-btn';
      pdfBtn.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" style="margin-right:6px;vertical-align:-3px;"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>Export Full Analysis to PDF';
